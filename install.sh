...
cp -r "$SCRIPT_DIR/skills/sales-advisor" "$SKILLS_DIR/"
cp -r "$SCRIPT_DIR/skills/client-sandbox" "$SKILLS_DIR/"
cp -r "$SCRIPT_DIR/skills/company-matcher" "$SKILLS_DIR/"
cp -r "$SCRIPT_DIR/skills/cc-connect-bridge" "$SKILLS_DIR/"
cp -r "$SCRIPT_DIR/skills/cc-weixin-bridge" "$SKILLS_DIR/"
echo "   finance-suite（金融投研 6 技能）"
echo "   sales-advisor（销售军师）"
echo "   client-sandbox（客户沙盘推演）"
echo "   company-matcher（上市公司业务匹配）"
...
mkdir -p "$QCLAW_WORKSPACE/memory/clients"
echo "   memory/customers/ （用户记忆）已就绪"
echo "   memory/clients/   （客户档案）已就绪"
