...
version: 1.0.0
description: >
  AI金融分析套件。7大技能：看票分析、宏观内参、麦肯锡报告、视频拆解、行业报告、集合竞价、投行研报。
...
7大专业分析技能，集成东方财富实时数据 + 双搜索引擎 + 视频字幕提取 + 投行研报入口。
...
| 集合竞价、涨停、选股信号 | 集合竞价 | [prompts/auction-analysis.md](prompts/auction-analysis.md) | `market_pulse` |
| 投行研报、今日Pitch、高盛、IMA、研报入口 | 投行研报 | [prompts/industry-report.md](prompts/industry-report.md) | scripts/research_reports.py |
...
3. **获取数据** → 调用对应的 **MCP 工具** 获取实时数据（不再执行本地脚本）
...
1) 获取研报来源入口（微信公众号 + IMA知识库）
   python3 scripts/research_reports.py --action sources

2) 获取精选研报列表
   python3 scripts/research_reports.py --action list --limit 10

3) 按关键词检索
   python3 scripts/research_reports.py --action search --keyword 今日Pitch
...
| **微信公众号+IMA知识库** | 投行研报入口与精选列表 | 入口需登录，精选列表本地维护 |
